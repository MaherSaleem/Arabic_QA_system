...
public class HelpersQT {
    ...
    public static ArrayList<Integer> getQuestionTypeUsingSVM(ArrayList<String> testingQuestions) throws IOException {
        ...
        if (ConfigM.VERBOSE_LOG) {
            for (int i = 0; i < result.size(); i++) {
                Logger.getInstance().log(ConfigM.LogFolders.PREPROCESSING + "/quesetion_types", "question is:" + testingQuestions.get(i));
                Logger.getInstance().log(ConfigM.LogFolders.PREPROCESSING + "/quesetion_types", "question type is:" + ConfigQT.QT_texts[result.get(i)]);
                Logger.getInstance().log(ConfigM.LogFolders.PREPROCESSING + "/quesetion_types", "=================================");
            }
        }
        return result;
    }
